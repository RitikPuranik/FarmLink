...
export class MarketDataService {
  ...
  async persist(record: SourceMarketRecord, client: PrismaClient | Prisma.TransactionClient = this.prisma) {
    ...
    if(record.sourceRecordId)await client.mandiPrice.upsert({where:{source_sourceRecordId:{source:record.source,sourceRecordId:record.sourceRecordId}},create,update});else await client.mandiPrice.upsert({where:{cropId_mandiId_observedDate_source:{cropId:crop.id,mandiId:mandi.id,observedDate:record.observedDate,source:record.source}},create,update});
    return { imported:true as const };
  }
  async run(records: AsyncIterable<SourceMarketRecord>, source: string, operation: "HISTORICAL_IMPORT" | "INCREMENTAL_SYNC") { const run=await this.prisma.marketDataImportRun.create({data:{source,operation}}); let read=0, imported=0, rejected=0; let newestObservedDate:Date|undefined; const diagnostics: string[]=[]; const processChunk=async(chunk:SourceMarketRecord[])=>this.prisma.$transaction(async tx=>{for(const record of chunk){try{const outcome=await this.persist(record,tx);if(outcome.imported){imported++;if(!newestObservedDate||record.observedDate>newestObservedDate)newestObservedDate=record.observedDate;}else{rejected++;diagnostics.push(`${record.commodity}: ${outcome.reason}`);}}catch(err){rejected++;diagnostics.push(`${record?.commodity ?? "record"}: ${err instanceof Error?err.message:"Invalid record"}`);}}});try { let chunk:SourceMarketRecord[]=[];for await (const record of records){read++;chunk.push(record);if(chunk.length===250){await processChunk(chunk);chunk=[];}}if(chunk.length)await processChunk(chunk);const status=rejected ? (imported ? "PARTIAL_SUCCESS" : "FAILED") : "SUCCEEDED"; await this.prisma.marketDataImportRun.update({where:{id:run.id},data:{status,recordsRead:read,recordsImported:imported,recordsSkipped:rejected,recordsRejected:rejected,validationFailures:rejected,diagnostics:diagnostics.slice(0,100),completedAt:new Date()}}); if(imported) await invalidateMarketCache(); return {runId:run.id,read,imported,rejected,newestObservedDate}; } catch(err) { await this.prisma.marketDataImportRun.update({where:{id:run.id},data:{status:"FAILED",recordsRead:read,recordsImported:imported,recordsRejected:rejected,validationFailures:rejected,diagnostics:diagnostics.slice(0,100),completedAt:new Date()}}); throw err; } }
}
